...
aggregate(Sepal.Length ~ Species, data = iris, mean) # most organized output
...
freq_sl <- sort(table(iris$Sepal.Length), decreasing = TRUE) # mode
freq_sl[1]
...
my_boxplot # o objeto te da todos os valores significantes do boxplot
...
# get the values except the outliers:
iris_wo_out = iris[!(iris$Sepal.Width %in% outliers), c("Sepal.Width", "Species")]
